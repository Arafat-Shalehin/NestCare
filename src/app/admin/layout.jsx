...
import { FaChartBar, FaCalendarCheck, FaHandsHelping, FaUsers, FaSignOutAlt, FaHome } from "react-icons/fa";
            ...
            <aside className="w-64 bg-(--color-surface) border-r border-(--color-border-subtle) flex flex-col fixed inset-y-0 z-50">
                <div className="p-6">
                    <Link href="/" className="flex items-center gap-3 mb-8 hover:opacity-80 transition-opacity">
                        <div className="w-8 h-8 rounded-lg bg-(--color-primary-600) flex items-center justify-center text-white font-bold">N</div>
                        <span className="text-xl font-bold tracking-tight text-(--color-text-main)">NestCare <span className="text-xs text-primary">Admin</span></span>
                    </Link>
                    ...
                    <nav className="space-y-1">
                        {menuItems.map((item) => {
                            ...
                            return (
                                ...
                                >
                                    <span className={`text-lg ${isActive ? "text-(--color-primary-600)" : "text-(--color-text-soft) group-hover:text-(--color-text-muted)"}`}>
                                        {item.icon}
                                    </span>
                                    {item.label}
                                    {isActive && (
                                        <motion.div layoutId="activeNav" className="ml-auto w-1.5 h-1.5 rounded-full bg-(--color-primary-600)" />
                                    )}
                                </Link>
                            );
                        })}
            ...
            <main className="flex-1 ml-64 p-8">
                <header className="mb-8 flex items-center justify-between">
                    <div className="space-y-1">
                        <h2 className="text-2xl font-bold text-(--color-text-main)">Admin Control Panel</h2>
                        <p className="text-sm text-(--color-text-muted)">Manage your caregiving network with precision.</p>
                    </div>
                    <div className="flex items-center gap-4">
                        <div className="text-right">
                            <p className="text-sm font-bold text-(--color-text-main)">{session.user.name}</p>
